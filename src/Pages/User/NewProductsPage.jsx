...
import { useNewReleasesMutation } from "../../Redux/Api/AdminApi";
...
const NewProductsPage = () => {
  const [NewReleases, { isLoading }] = useNewReleasesMutation();
  const [data, setdata] = useState();
  useEffect(() => {
    window.scrollTo(0, 0);
    (async () => {
      const res = await NewReleases();
      if (res.data) {
  ...
  return (
    <div className="mt-36 h-fit w-full overflow-hidden phoneLarge:mt-24">
      <div className="pt-5 pl-[5%]">
        <h1 className="text-3xl font-header mb-5">New Releases</h1>
      </div>
      {isLoading && <Loading />}
      {data?.length > 0 ? (
